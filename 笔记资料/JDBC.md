...
- [一、JDBC的体系结构](#一jdbc的体系结构)
  - [1.面向应用的API：Java API，抽象接口，供应用程序开发人员使用(连接数据库，执行SQL语句，获得结果)](#1面向应用的apijava-api抽象接口供应用程序开发人员使用连接数据库执行sql语句获得结果)
  - [2.面向数据库的API：Java DRIVER API，供开发商开发数据库驱动用](#2面向数据库的apijava-driver-api供开发商开发数据库驱动用)
- [二、JDBC程序编写步骤](#二jdbc程序编写步骤)
- [三、获取数据库连接](#三获取数据库连接)
  - [{一}方式一：](#一方式一)
  - [{二}方式二](#二方式二)
  - [{三}方式三](#三方式三)
  - [{四}方式四](#四方式四)
  - [{五}方式五](#五方式五)
- [四、使用PreparedStatement操作数据库](#四使用preparedstatement操作数据库)
  - [{一}使用Statement操作数据库表的弊端和使用PreparedStatement操作数据库表的优势](#一使用statement操作数据库表的弊端和使用preparedstatement操作数据库表的优势)
  - [{二}使用PreparedStatement(从Statement扩展而来)替代Statement](#二使用preparedstatement从statement扩展而来替代statement)
  - [{三}ORM编程思想(object relational mapping)](#三orm编程思想object-relational-mapping)
  - [{四}查询](#四查询)
  - [{五}操作Blob类型](#五操作blob类型)
  - [{六}批量插入](#六批量插入)
  - [{七}加入事务的使用](#七加入事务的使用)
- [五、DAO](#五dao)
- [六、数据库连接池](#六数据库连接池)
- [七、Apache-DBUtils](#七apache-dbutils)
